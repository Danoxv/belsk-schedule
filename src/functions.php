...
function resolveExcelConfig(Worksheet $sheet, array $config)
{
    ...
    $config = array_merge([
        ...
        'groupNamesRow'     => null,
        'firstScheduleRow'  => null,

        'classHourLessonColumn' => null,
    ]);
    ...
    foreach ($columns as $column) {
        foreach ($rows as $row) {
            if (isExcelConfigProcessable($config)) {
                break(2);
            }
            ...
            if (in_array($cellValue, $dayWords)) {
                ...
                $config['groupNamesRow']    = $row;
                $config['firstScheduleRow'] = nextRow($config['groupNamesRow']);
            } elseif (in_array($cellValue, $timeWords)) {
                $config['timeCol']          = $column;
                $config['firstGroupCol']    = nextColumn($config['timeCol']);
                $config['groupNamesRow']    = $row;
                $config['firstScheduleRow'] = nextRow($config['groupNamesRow']);
            } else if (isClassHourLesson($rawCellValue)) {
                $config['classHourLessonColumn'] = $column;
            }
        }
    }

    if ($config['classHourLessonColumn'] === null) {
        $config['classHourLessonColumn'] = false;
    }

    return $config;
}

function isExcelConfigProcessable(array $config): bool
{
